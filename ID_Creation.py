...
def show_login_page():
    ...
    center = st.selectbox("Select Center", ["KOLKATA", "INDORE-TARUS", "MYSORE-TTBS", "BHOPAL-TTBS", "RANCHI-AYUDA", "BHOPAL-MGM", "COIM-HRHNXT", "NOIDA-ICCS", "HYD-CORPONE", "VIJAYAWADA-TTBS"])
    ...
    if st.button("Login"):
        if not batch_no:
            ...
            st.session_state.data = []  # Initialize data for rows
...
def is_valid_email(email):
    email_regex = r"(^[a-zA-Z0-9_.+-]+@[a-zAZA-Z0-9-]+\.[a-zA-Z0-9-.]+$)"
...
# Function to show the form and handle data
def show_form():
    ...
    if st.session_state.data:
        ...
        columns = len(df.columns) + 1  # Add one more column for the delete button
        ...
        for i, row in df.iterrows():
            ...
            if rows[-1].button(f"Delete Row {i+1}", key=f"delete_{i}"):
                st.session_state.data.pop(i)
                st.success(f"Row {i+1} deleted successfully!")
                break  # Stop iteration after the deletion
    ...
    # Submit button to upload data to Google Sheets
    if st.button("Submit"):
        if st.session_state.data:
